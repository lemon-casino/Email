...
class ProjectRuntimeTests(unittest.TestCase):
    ...
    def test_accounts_api_imports_outlook_oauth_bulk_lines(self):
        lines = [
            'route-one@outlook.com----fake-password-a----11111111-1111-4111-8111-111111111111----0.AXEA_fake_refresh_a',
            'route-two@hotmail.com----fake-password-b----0.AXEA_fake_refresh_b----22222222-2222-4222-8222-222222222222',
        ]

        payload = self._import_outlook_oauth_accounts(lines)
        ...
        with self.app.app_context():
            ...
            rows = {
                ...
                for row in db.execute(
                    '''
                    SELECT email, password, client_id, refresh_token, account_type, provider,
                           imap_host, imap_port, imap_password, status, forward_enabled
                    FROM accounts
                    ORDER BY email
                    '''
                ).fetchall()
            }
            first_account = web_outlook_app.get_account_by_email('route-one@outlook.com')
            second_account = web_outlook_app.get_account_by_email('route-two@hotmail.com')

        self.assertEqual(set(rows), {'route-one@outlook.com', 'route-two@hotmail.com'})
        first_row = rows['route-one@outlook.com']
        self.assertEqual(first_row['client_id'], '11111111-1111-4111-8111-111111111111')
        self.assertNotEqual(first_row['password'], 'fake-password-a')
        self.assertNotEqual(first_row['refresh_token'], '0.AXEA_fake_refresh_a')
        self.assertEqual(first_row['account_type'], 'outlook')
        self.assertEqual(first_row['provider'], 'outlook')
        self.assertEqual(first_row['imap_host'], web_outlook_app.IMAP_SERVER_NEW)
        self.assertEqual(first_row['imap_port'], web_outlook_app.IMAP_PORT)
        self.assertEqual(first_row['imap_password'], '')
        self.assertEqual(first_row['status'], 'active')
        self.assertEqual(first_row['forward_enabled'], 0)
        self.assertEqual(first_account['password'], 'fake-password-a')
        self.assertEqual(first_account['refresh_token'], '0.AXEA_fake_refresh_a')
        self.assertEqual(second_account['client_id'], '22222222-2222-4222-8222-222222222222')
        self.assertEqual(second_account['refresh_token'], '0.AXEA_fake_refresh_b')

    def test_accounts_api_returns_explicit_error_for_invalid_json(self):
        response = self.client.post(
            '/api/accounts',
            data='{"account_string":',
        ...
        self.assertIn('JSON 格式错误', payload['error'])

    @unittest.skipUnless(getattr(web_outlook_app, 'CSRF_AVAILABLE', False), 'Flask-WTF not installed')
    def test_accounts_api_import_is_not_blocked_by_csrf(self):
        original_csrf_enabled = self.app.config.get('WTF_CSRF_ENABLED')
        self.app.config['WTF_CSRF_ENABLED'] = True
        try:
            payload = self._import_outlook_oauth_accounts([
                'csrf-free@outlook.com----fake-password----44444444-4444-4444-8444-444444444444----0.AXEA_fake_refresh_csrf',
            ])
        finally:
        ...
        with self.app.app_context():
            account = web_outlook_app.get_account_by_email('csrf-free@outlook.com')
        self.assertIsNotNone(account)
        self.assertEqual(account['provider'], 'outlook')

    @unittest.skipUnless(getattr(web_outlook_app, 'CSRF_AVAILABLE', False), 'Flask-WTF not installed')
    def test_mail_action_apis_are_not_blocked_by_csrf(self):
        original_csrf_enabled = self.app.config.get('WTF_CSRF_ENABLED')
        self.app.config['WTF_CSRF_ENABLED'] = True
        try:
            self._import_outlook_oauth_accounts([
                'csrf-actions@outlook.com----fake-password----55555555-5555-4555-8555-555555555555----0.AXEA_fake_refresh_actions',
            ])
            ...
            mark_result = {
                ...
                'success_count': 1,
                'failed_count': 0,
                'updated_ids': ['message-1'],
                'errors': [],
            }
            with patch.object(web_outlook_app, 'mark_graph_items_read', return_value=mark_result):
                mark_response = self.client.post('/api/emails/mark-read', json={
                    'email': 'csrf-actions@outlook.com',
                    'method': 'graph',
                    'folder': 'inbox',
                    'items': [{
                        'id': 'message-1',
            ...
            delete_result = {
                ...
                'success_count': 1,
                'failed_count': 0,
                'deleted_ids': ['message-1'],
                'errors': [],
            }
            with patch.object(web_outlook_app, 'delete_emails_graph', return_value=delete_result):
                delete_response = self.client.post('/api/emails/delete', json={
                    'email': 'csrf-actions@outlook.com',
                    'ids': ['message-1'],
                })

            with patch.object(web_outlook_app, 'is_normal_mail_local_retention_enabled', return_value=False):
                retain_response = self.client.post('/api/emails/retain-bodies', json={
                    'email': 'csrf-actions@outlook.com',
                    'folder': 'inbox',
                    'items': [{
                        'id': 'message-1',
        ...
        self.assertEqual(mark_payload['updated_ids'], ['message-1'])
        self.assertNotIn('csrf_error', mark_payload)
        ...
        retain_payload = retain_response.get_json()
        self.assertFalse(retain_payload['success'])
        self.assertIn('普通邮箱本地保留未启用', retain_payload['error'])
        self.assertNotIn('csrf_error', retain_payload)

    @unittest.skipUnless(getattr(web_outlook_app, 'CSRF_AVAILABLE', False), 'Flask-WTF not installed')
    def test_account_management_apis_are_not_blocked_by_csrf(self):
        original_csrf_enabled = self.app.config.get('WTF_CSRF_ENABLED')
        original_csrf_check_default = self.app.config.get('WTF_CSRF_CHECK_DEFAULT')
        self.app.config['WTF_CSRF_ENABLED'] = True
        self.app.config['WTF_CSRF_CHECK_DEFAULT'] = True
        try:
            status_account_id = self._insert_account('csrf-status@outlook.com')
            batch_account_id = self._insert_account('csrf-batch@outlook.com')
            target_group_id = self._create_group('CSRF 目标分组')

            status_response = self.client.put(
                f'/api/accounts/{status_account_id}',
                json={'status': 'inactive'},
            )
            group_response = self.client.post('/api/accounts/batch-update-group', json={
                'account_ids': [batch_account_id],
                'group_id': target_group_id,
            })
            forwarding_response = self.client.post('/api/accounts/batch-update-forwarding', json={
                'account_ids': [batch_account_id],
                'forward_enabled': True,
            })
            proxy_response = self.client.post('/api/accounts/batch-update-proxy', json={
                'account_ids': [batch_account_id],
                'proxy_url': 'socks5://127.0.0.1:1080',
            })
            stream_task_response = self.client.post('/api/accounts/refresh-selected-stream', json={
                'account_ids': [batch_account_id],
            })
            with patch.object(
                web_outlook_app,
                'refresh_outlook_account_token',
                return_value={'success': True, 'message': 'Token 刷新成功'},
            ):
                refresh_response = self.client.post(f'/api/accounts/{batch_account_id}/refresh')
                refresh_selected_response = self.client.post('/api/accounts/refresh-selected', json={
                    'account_ids': [batch_account_id],
                })
            delete_response = self.client.post('/api/accounts/batch-delete', json={
                'account_ids': [batch_account_id],
            })
        finally:
            self.app.config['WTF_CSRF_ENABLED'] = original_csrf_enabled
        ...
        self.assertTrue(group_response.get_json()['success'])
        self.assertTrue(forwarding_response.get_json()['success'])
        self.assertTrue(proxy_response.get_json()['success'])
        self.assertTrue(stream_task_response.get_json()['success'])
        self.assertTrue(refresh_response.get_json()['success'])
        self.assertTrue(refresh_selected_response.get_json()['success'])
    ...
    @unittest.skipUnless(getattr(web_outlook_app, 'CSRF_AVAILABLE', False), 'Flask-WTF not installed')
    def test_outlook_oauth_and_upload_apis_are_not_blocked_by_csrf(self):
        original_csrf_enabled = self.app.config.get('WTF_CSRF_ENABLED')
        original_csrf_check_default = self.app.config.get('WTF_CSRF_CHECK_DEFAULT')
        self.app.config['WTF_CSRF_ENABLED'] = True
        self.app.config['WTF_CSRF_CHECK_DEFAULT'] = True
        try:
            with patch.object(
                ...
                return_value={'success': False, 'error': 'invalid redirected url'},
            ):
                exchange_response = self.client.post('/api/oauth/exchange-token', json={
                    'redirected_url': 'bad-url',
                })

            reauthorize_response = self.client.post('/api/accounts/999999/reauthorize', json={
                'redirected_url': 'bad-url',
            })

            upload_response = self.client.post('/api/outlook-upload-accounts', json={
                'email': 'csrf-upload@outlook.com',
                'password': 'upload-password',
            })
            upload_payload = upload_response.get_json()
            upload_account_id = upload_payload.get('account', {}).get('id')

            graph_response = self.client.post('/api/oauth/graph-extract-token', json={
                'account_id': 999999,
            })
            delete_upload_response = self.client.delete(
                f'/api/outlook-upload-accounts/{upload_account_id}'
            )
        finally:
            self.app.config['WTF_CSRF_ENABLED'] = original_csrf_enabled
            if original_csrf_check_default is None:
        ...
        self.assertFalse(exchange_response.get_json()['success'])
        self.assertEqual(reauthorize_response.status_code, 404)
        self.assertTrue(upload_response.get_json()['success'])
        self.assertEqual(graph_response.status_code, 404)
        self.assertTrue(delete_upload_response.get_json()['success'])

    def test_outlook_imported_account_can_fetch_mail_with_graph(self):
        self._import_outlook_oauth_accounts([
            'route-graph@outlook.com----fake-password----11111111-1111-4111-8111-111111111111----0.AXEA_fake_refresh_graph',
        ])
        graph_result = {
            'success': True,
            'emails': [{
                ...
                'toRecipients': [{'emailAddress': {'address': 'route-graph@outlook.com'}}],
        ...
        with patch.object(web_outlook_app, 'is_normal_mail_local_retention_enabled', return_value=False), \
                patch.object(web_outlook_app, 'get_emails_graph', return_value=graph_result) as graph_mock, \
                patch.object(web_outlook_app, 'get_emails_imap_with_server') as imap_mock:
            response = self.client.get('/api/emails/route-graph@outlook.com?folder=inbox&skip=2&top=3')

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertTrue(payload['success'])
        self.assertEqual(payload['method'], 'Graph API')
        self.assertEqual(payload['request_method'], 'graph')
        self.assertEqual(payload['emails'][0]['id'], 'graph-message-1')
        self.assertEqual(payload['emails'][0]['id_mode'], 'graph')
        self.assertEqual(payload['requested_email'], 'route-graph@outlook.com')
        self.assertEqual(payload['resolved_email'], 'route-graph@outlook.com')
        graph_mock.assert_called_once()
        self.assertEqual(
            graph_mock.call_args.args[:5],
            ('11111111-1111-4111-8111-111111111111', '0.AXEA_fake_refresh_graph', 'inbox', 2, 3),
        )
        imap_mock.assert_not_called()

    def test_outlook_imported_account_falls_back_to_new_imap_when_graph_fails(self):
        self._import_outlook_oauth_accounts([
            'route-imap@outlook.com----fake-password----33333333-3333-4333-8333-333333333333----0.AXEA_fake_refresh_imap',
        ])
        imap_result = {
            'success': True,
            'emails': [{
                'id': '42',
                'subject': 'IMAP fallback subject',
                'from': 'sender@example.com',
                'to': 'route-imap@outlook.com',
                'date': '2026-06-30T09:00:00Z',
                'body_preview': 'IMAP preview',
                'id_mode': 'uid',
            }],
            'has_more': False,
        }
        ...
        with patch.object(web_outlook_app, 'is_normal_mail_local_retention_enabled', return_value=False), \
                patch.object(web_outlook_app, 'get_emails_graph', return_value={'success': False, 'error': 'Graph failed'}), \
                patch.object(web_outlook_app, 'get_emails_imap_with_server', return_value=imap_result) as imap_mock:
            response = self.client.get('/api/emails/route-imap@outlook.com?folder=junkemail&top=5')

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertTrue(payload['success'])
        self.assertEqual(payload['method'], 'IMAP (New)')
        self.assertEqual(payload['request_method'], 'imap')
        self.assertEqual(payload['emails'][0]['id'], '42')
        self.assertEqual(payload['emails'][0]['folder'], 'junkemail')
        imap_mock.assert_called_once()
        self.assertEqual(
            imap_mock.call_args.args[:7],
            (
                'route-imap@outlook.com',
                '33333333-3333-4333-8333-333333333333',
                '0.AXEA_fake_refresh_imap',
                'junkemail',
                0,
                5,
                web_outlook_app.IMAP_SERVER_NEW,
            ),
        )
...
class FrontendTimezoneBootstrapTests(unittest.TestCase):
    ...
    def test_temp_email_import_cloudflare_channel_examples_and_tags(self):
        ...
        self.assertIn('<option value="outlook">Outlook OAuth</option>', dialog_html)
        ...
        self.assertIn("const isOutlookChannel = isTempGroup && channel === 'outlook';", groups_js)
        self.assertIn('Outlook 会作为正式邮箱导入到普通分组', groups_js)
        self.assertIn('function looksLikeOutlookOauthImport(input)', settings_js)
        self.assertIn("const shouldAutoRouteOutlook = isTempGroup && tempProvider !== 'outlook' && looksLikeOutlookOauthImport(input);", settings_js)
        self.assertIn("const isOutlookChannelImport = isTempGroup && (tempProvider === 'outlook' || shouldAutoRouteOutlook);", settings_js)
        self.assertIn("provider: isOutlookChannelImport ? 'outlook' : provider", settings_js)
        self.assertIn("const isTagField = !!field.querySelector('#importTagDropdown');", groups_js)
        self.assertIn("field.style.display = isTempGroup && !isOutlookChannel ? (isTagField ? '' : 'none') : '';", groups_js)
    ...
    def test_temp_email_channel_toolbar_can_switch_to_outlook(self):
        layout_html = pathlib.Path(ROOT_DIR, 'templates', 'partials', 'index', 'layout.html').read_text(encoding='utf-8')
        groups_js = pathlib.Path(ROOT_DIR, 'static', 'js', 'index', '02-groups.js').read_text(encoding='utf-8')

        self.assertIn('data-provider="outlook"', layout_html)
        self.assertIn("onclick=\"filterTempEmailByProvider('outlook')\"", layout_html)
        self.assertIn('async function switchToOutlookMailboxChannel()', groups_js)
        self.assertIn("if (provider === 'outlook')", groups_js)
