        ...
        function looksLikeOutlookOauthImport(input) {
            ...
            return String(input || '').split('\n').some(line => {
                ...
                return clientIdPattern.test(parts[2]) || clientIdPattern.test(parts[3]);
            });
        }
        ...
        async function addAccount() {
            ...
            const shouldAutoRouteOutlook = isTempGroup && tempProvider !== 'outlook' && looksLikeOutlookOauthImport(input);
            const isOutlookChannelImport = isTempGroup && (tempProvider === 'outlook' || shouldAutoRouteOutlook);
            const destinationGroupId = isOutlookChannelImport && typeof getDefaultNormalGroupId === 'function'
                ? getDefaultNormalGroupId()
                : groupId;
            ...
            if (isOutlookChannelImport && !destinationGroupId) {
                showToast('请先创建普通邮箱分组', 'error');
                return;
            }
            if (isTempGroup && !isOutlookChannelImport && tempProvider === 'cloudflare' && !cloudflareChannelId) {
            ...
            try {
                ...
                let response;
                if (isTempGroup && !isOutlookChannelImport) {
                    ...
                    response = await fetch('/api/accounts', {
                        ...
                        body: JSON.stringify({
                            ...
                            provider: isOutlookChannelImport ? 'outlook' : provider,
                            imap_host: imapHost,
                ...
                if (data.success) {
                    ...
                    hideAddAccountModal();
                    delete accountsCache[destinationGroupId];
                    if (isOutlookChannelImport) {
                        currentGroupId = destinationGroupId;
                        localStorage.setItem('outlook_last_group_id', String(destinationGroupId));
                    }
                    await loadGroups();
                    if (isTempGroup && !isOutlookChannelImport) {
                        await loadTempEmails(true);
                    } else {
                        await loadAccountsByGroup(destinationGroupId, true);
                    }
                } else {
